...
pub(super) struct DeserializerState<'db, R> {
    ...
    /// A map of i32 ref id to final instance referent.
    ...
    /// The PRNT chunk, of which there should only be one.
    /// Decoding of the PRNT chunk is deferred until finish().
    ...
    pub(super) fn decode_inst_chunk(&mut self, mut chunk: &[u8]) -> Result<(), InnerError> {
        ...
        let instances = chunk
            ...
            .map(|referent| {
                ...
                self.referent_by_ref.insert(referent, builder.referent());
                (
                    referent,
                    Instance {
                        builder,
...
rbx-dom may require changes to fully support this property. Please open an issue at https://github.com/rojo-rbx/rbx-dom/issues and show this warning.",
            ...
            Type::Int32 => match canonical_type {
                VariantType::Int32 => {
                    let values = chunk.read_interleaved_i32_array(type_info.instances.len())?;
            ...
            Type::Float32 => match canonical_type {
                VariantType::Float32 => {
                    let values = chunk.read_interleaved_f32_array(type_info.instances.len())?;
            ...
            Type::UDim => match canonical_type {
                VariantType::UDim => {
                    let scales = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let offsets = chunk.read_interleaved_i32_array(type_info.instances.len())?;
            ...
            Type::UDim2 => match canonical_type {
                VariantType::UDim2 => {
                    let prop_count = type_info.instances.len();
            ...
            Type::Color3 => match canonical_type {
                VariantType::Color3 => {
                    let r = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let g = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let b = chunk.read_interleaved_f32_array(type_info.instances.len())?;
            ...
            Type::Vector2 => match canonical_type {
                VariantType::Vector2 => {
                    let x = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let y = chunk.read_interleaved_f32_array(type_info.instances.len())?;
            ...
            Type::Vector3 => match canonical_type {
                VariantType::Vector3 => {
                    let x = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let y = chunk.read_interleaved_f32_array(type_info.instances.len())?;
                    let z = chunk.read_interleaved_f32_array(type_info.instances.len())?;
            ...
            Type::CFrame => match canonical_type {
                VariantType::CFrame => {
                    ...
                    for (value, (_, instance)) in values.zip(instances) {
                        add_property(instance, &property, value.into());
                    }
                }
                invalid_type => {
            ...
            Type::Ref => match canonical_type {
                VariantType::Ref => {
                    ...
                    for (value, (_, instance)) in values.zip(&mut type_info.instances) {
                        let rbx_value = if let Some(&referent) = self.referent_by_ref.get(&value) {
                            referent
                        } else {
                            Ref::none()
                        };
            ...
            Type::Rect => match canonical_type {
                VariantType::Rect => {
                    let len = type_info.instances.len();
            ...
            Type::Int64 => match canonical_type {
                VariantType::Int64 => {
                    let values = chunk.read_interleaved_i64_array(type_info.instances.len())?;
                    ...
                    for (ty, (_, instance)) in values.zip(&mut type_info.instances) {
                        let value = match ty {
                            ...
                            2 => {
                                ...
                                if let Some(&referent) = self.referent_by_ref.get(&read_value) {
    ...
    pub(super) fn ingest_prnt_chunk(&mut self, chunk: Vec<u8>) -> Result<(), InnerError> {
        let replaced = self.prnt_chunk.replace(DeferredChunk {
            chunk,
            start_position: 0,
        });
    ...
    pub(super) fn finish(mut self) -> Result<WeakDom, InnerError> {
        ...
        let Some(prnt_chunk) = self.prnt_chunk else {
        ...
        for type_info in self.type_infos.into_values() {
